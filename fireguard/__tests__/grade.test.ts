...
function ast(
  partial: Partial<AstGateResult> & { pass: boolean }
): AstGateResult {
  return {
    name: "ast",
    assertionCount: 10,
...
function flake(
  partial: Partial<FlakeGateResult> & { pass: boolean }
): FlakeGateResult {
  return {
    name: "flake",
    runs: 100,
...
function mutation(
  partial: Partial<MutationGateResult> & { pass: boolean }
): MutationGateResult {
  return {
    name: "mutation",
    killed: 8,
...
describe("computeGrade", () => {
  it("forces F when any hard gate fails", () => {
    const grade = computeGrade({
      ast: ast({
        pass: false,
        findings: [{ file: "a.ts", line: 1, rule: "empty", message: "x" }],
      }),
  ...
  it("scores A when mutation is omitted after hard passes elsewhere", () => {
    const grade = computeGrade({
      ast: ast({ pass: true, mockToAssertRatio: 0, tautologicalRatio: 0 }),
      flake: flake({ pass: true }),
    });
