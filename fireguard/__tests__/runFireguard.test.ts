...
import { runFireguard } from "../src/runFireguard";
  ...
  it("fails closed with F when modules change without graded test updates", async () => {
    const report = await runFireguard({
      ...
      getDiffEntries: async () => [
        { status: "M", path: "src/main/steamCmd.ts" },
      ],
      readFile: async () => "",
...
`;
  ...
  it("fail-fasts on AST before flake/mutation", async () => {
    const runOnce = jest.fn(async () => ({ ok: true }));
    const applyAndTest = jest.fn(async () => ({ ok: false }));
    const slop = `
const fn = jest.fn(() => 1);
...
`;
    const report = await runFireguard({
      ...
      readFile: async (path) =>
        path.endsWith(".test.ts") ? slop : "export const x = 1;",
      runOnce,
      applyAndTest,
    });
    expect(report.grade.letter).toBe("F");
...
`;
    const report = await runFireguard({
      ...
      readFile: async (path) =>
        path.endsWith(".test.ts") ? solidTest : solidMod,
      runOnce: async () => ({ ok: true }),
      applyAndTest: async () => ({ ok: false }),
    });
    expect(report.gates.ast?.pass).toBe(true);
