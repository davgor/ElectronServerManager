...
import { FIREGUARD_COMMENT_MARKER } from "../src/prComment";
...
describe("runCli --comment-pr", () => {
  it("posts markdown grade comment and still exits 0 for skipped A", async () => {
    const postComment = jest.fn(async () => "created" as const);
    ...
    const code = await runCli({
      ...
      env: {
        GITHUB_TOKEN: "t",
        GITHUB_REPOSITORY: "davgor/ElectronServerManager",
  ...
  it("returns 2 when comment-pr lacks token/repo/pr", async () => {
    const code = await runCli({
      ...
      stdout: () => undefined,
      stderr: () => undefined,
      postComment: jest.fn(async () => "created" as const),
      runFireguardFn: async () => skippedReport,
    });
    expect(code).toBe(2);
  });
