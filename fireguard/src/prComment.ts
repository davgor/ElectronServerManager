import { formatHumanReport } from "./report";
...
export function formatPrMarkdown(report: FireguardReport): string {
  ...
  if (report.skipped) {
    ...
    lines.push("");
    lines.push(
      "Added/modified Jest unit tests vs the base ref are graded for mock/assert quality, flake isolation, and mutation kill rate on changed modules. Module-only diffs without test updates fail closed with F."
    );
    return `${lines.join("\n")}\n`;
  }
  ...
  if (mutation) {
    ...
    for (const survivor of mutation.survivors.slice(0, 10)) {
      lines.push(
        `  - survivor \`${survivor.file}:${survivor.line}\` ${survivor.description}`
      );
    }
  }
...
export async function upsertPrComment(options: {
  ...
  const existing = comments.find((comment) =>
    comment.body.includes(FIREGUARD_COMMENT_MARKER)
  );
  if (existing) {
...
export async function postPrCommentViaGithub(options: {
  ...
  if (!owner || !repo) {
    throw new Error(
      `Invalid repository "${options.repository}" (expected owner/repo)`
    );
  }
  ...
  return upsertPrComment({
    ...
    listComments: async () => {
      ...
      while (page <= 10) {
        ...
        if (!response.ok) {
          throw new Error(
            `list comments failed: ${response.status} ${await response.text()}`
          );
        }
    ...
    createComment: async (body) => {
      const response = await fetchImpl(
        `${api}/issues/${options.pullNumber}/comments`,
        {
          method: "POST",
      ...
      if (!response.ok) {
        throw new Error(
          `create comment failed: ${response.status} ${await response.text()}`
        );
      }
    },
    updateComment: async (id, body) => {
      ...
      if (!response.ok) {
        throw new Error(
          `update comment failed: ${response.status} ${await response.text()}`
        );
      }
    },
  });
