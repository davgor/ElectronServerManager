...
import type { AstFinding, AstGateResult, FireguardConfig } from "../types";
...
/** Mock-framework namespaces whose `*.fn` / `*.mock` calls count as mocks. */
...
function isMockNamespaceIdentifier(expr: ts.Expression): boolean {
  return ts.isIdentifier(expr) && MOCK_NAMESPACES.has(expr.text);
}
...
function collectsMockCall(node: ts.CallExpression): boolean {
  const expr = node.expression;
  // Only count `jest.*` / `vi.*` mock APIs — not arbitrary `.fn` / `.mock` property access.
  if (ts.isPropertyAccessExpression(expr)) {
    return (
      isMockNamespaceIdentifier(expr.expression) &&
      MOCK_PROPERTY_NAMES.has(expr.name.text)
    );
  }
  return false;
}
...
function getExpectRootArgument(
  node: ts.CallExpression
): ts.Expression | undefined {
  return node.arguments[0];
}

function expressionLooksMocked(
  expr: ts.Expression | undefined,
  mockIdents: Set<string>
): boolean {
...
export function runAstGate(options: {
  ...
  for (const file of options.files) {
    ...
    if (
      metrics.assertionCount < options.config.thresholds.minAssertionsPerFile
    ) {
      findings.push({
        file: file.path,
        line: 1,
  ...
  const mockToAssertRatio =
    assertionCount === 0 ? mockCount : mockCount / assertionCount;
  const tautologicalRatio =
    assertionCount === 0 ? 0 : tautologicalCount / assertionCount;

  if (
    assertionCount > 0 &&
    mockToAssertRatio > options.config.thresholds.maxMockToAssertRatio
  ) {
    findings.push({
  ...
  if (
    assertionCount > 0 &&
    tautologicalRatio > options.config.thresholds.maxTautologicalRatio
  ) {
    findings.push({
      file: options.files[0]?.path ?? "(suite)",
      line: 1,
