...
import { formatHumanReport, formatJsonReport } from "./report";
import { runFireguard, type RunFireguardDeps } from "./runFireguard";
import type { FireguardReport } from "./types";
...
export interface CliOptions {
  ...
  /** Test seam — overrides the default git/jest-backed runner. */
...
export async function runCli(options: CliOptions = {}): Promise<number> {
  ...
  const readFile =
    options.readFileSyncFn ??
    ((path, encoding) => readFileSync(path, encoding));
  const writeFile =
    options.writeFileSyncFn ??
    ((path, data) => writeFileSync(path, data, "utf8"));
  const appendFile =
    options.appendFileSyncFn ??
    ((path, data) => appendFileSync(path, data, "utf8"));
...
Grades added or modified unit tests (git diff vs baseRef, default main) through:
  ...
  Gate 2  Flakiness (isolated re-runs, 0% flake; fail-fast)
...
`);
  ...
  try {
    ...
    const runOnce = createTestCommandRunner({ cwd, config });
  ...
  if (stepSummary) {
    try {
      ...
      const message = error instanceof Error ? error.message : String(error);
      stderr(
        `fireguard warning: could not write GITHUB_STEP_SUMMARY: ${message}\n`
      );
    }
  }
  ...
  if (commentPr) {
    ...
    const pullNumber =
      (env.FIREGUARD_PULL_NUMBER
        ? Number(env.FIREGUARD_PULL_NUMBER)
        : undefined) ??
      resolvePullNumberFromEvent(env.GITHUB_EVENT_PATH, (path) =>
        readFile(path, "utf8")
      );
    ...
    if (!token || !repository || !pullNumber) {
      ...
      try {
        ...
        stdout(
          `fireguard: PR comment ${action} (grade ${report.grade.letter})\n`
        );
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
