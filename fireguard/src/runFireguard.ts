import { runAstGate } from "./gates/astGate";
import { runFlakeGate } from "./gates/flakeGate";
import { runMutationGate } from "./gates/mutationGate";
import { computeGrade } from "./grade";
import { resolveGitScope } from "./gitScope";
import type {
  DiffEntry,
  FireguardConfig,
  FireguardReport,
...
export async function runFireguard(
  deps: RunFireguardDeps
): Promise<FireguardReport> {
  ...
  if (scope.gradedTestFiles.length === 0 && scope.changedModules.length > 0) {
    return {
      grade: {
        ...
        score: 0,
        reasons: [
          "production modules changed without graded unit test updates",
        ],
      },
      gates: {},
      scope: scopePayload,
