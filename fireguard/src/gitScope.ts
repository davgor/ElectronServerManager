...
import type { DiffEntry, FireguardConfig, GitScope } from "./types";

function matchesAny(path: string, patterns: string[]): boolean {
  return patterns.some((pattern) =>
    minimatch(path, pattern, { dot: true, matchBase: false })
  );
}
...
function isProductionModule(path: string, config: FireguardConfig): boolean {
  ...
  // Test helpers (setup, fixtures, non-test files under __tests__) are not
  // production modules in this repo.
  if (path.startsWith("src/__tests__/")) return false;
