...
Legacy alias: `.claude/skills/antagonistic-pr-review/SKILL.md` (same gate).
...
workflows, release, or agent skills — **including PRs you yourself just authored**.
...
3. Attack these angles (skip only if truly N/A, and say why):
   ...
   - **Gate evasion** — can an agent skip fireguard / lint / tests / diff-coverage / red-team by shaping the diff?
   - **Test honesty** — tautologies, missing edge cases, tests that cannot fail
   - **Security** — secrets, unsafe `spawn`/`shell`, token misuse, path traversal, XSS
   - **Electron** — `nodeIntegration`, missing `contextIsolation`, privileged preload leaks, widened IPC channels, generic exec channels
   - **IPC contract** — `src/types/ipc.ts`, preload allowlist, and main handlers staying in sync; structured result shapes preserved
   - **CI reality** — shallow clones, permissions, flake budget lies, non-failing continues, kickback/release check names still matching
...
- Electron main/preload/IPC: <what you tried, or N/A>
- Renderer (React): <what you tried, or N/A>
```
...
- Re-run the delivery verification gate after fixes (see
  `.claude/skills/delivery-standards/SKILL.md` and `.ai-instructions.md`).
...
- Does **not** replace fireguard, lint, unit tests, diff-coverage, or builds.
