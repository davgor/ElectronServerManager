...
Mirrors `.cursor/skills/complete-ticket/SKILL.md` — keep both in sync when changing workflow rules.
...
- Read `README.md`, `ARCHITECTURE.md`, and `.ai-instructions.md` at the repo root — they cover architecture, IPC boundaries, and the mandatory lint/format workflow.
...
- Check whether prerequisite tickets this one depends on are actually done (e.g. don't implement server start/stop UI against IPC handlers that don't exist yet). If a hard dependency is missing, say so and ask whether to do the dependency first instead of improvising a stand-in.

## 3. Implement TDD-first

This repo's standing rule: tests are written before the implementation that satisfies them, for everything in `src/main/` (Steam detection, process management, backups), `src/preload/`, and testable renderer logic. For each acceptance criterion that names a behavior:
...
- Keep `src/main/` logic free of React/DOM imports; keep `src/renderer/` free of direct Node/Electron main-process APIs (use typed IPC via preload).
- Electron security baseline (contextIsolation on, narrow typed IPC, no generic exec/shell channel) is never weakened by a ticket's implementation.
...
2. Launch the real app (`npm start`, or `npm run build && npm run electron-build && electron .` with the Vite dev server if needed) and exercise the new IPC path inside it — not just confirm the window opens.
3. Re-run `npm test` afterward to confirm the suite still passes.

Skip this step only for tickets that don't touch `main`/`preload` IPC (e.g. pure renderer UI with mocked IPC in tests, docs, ticket-board work) — say so rather than running it pointlessly.
...
1. **Resolve the full scope up front.** Find the epic file and every backlog/in-progress sub-ticket under it (or just the named range, if the user gave one). Read all of them plus `README.md` and `ARCHITECTURE.md` before assigning any work — you need the whole set in view to spot cross-ticket dependencies.
...
4. **Fan out independent sub-tickets to sub-agents.** For each parallelizable group, launch one sub-agent per sub-ticket (or a small cluster of trivially related ones) in a *single message* so they run concurrently. Each sub-agent prompt must be self-contained:
   - The full text of the ticket(s) it owns (description + acceptance criteria) and any README/ARCHITECTURE excerpt it needs.
   - The exact files it owns to create/edit, so two sub-agents never touch the same file.
   - The standing rules from section 3 (TDD-first, ESLint strict, main/renderer IPC boundary).
   - An instruction to self-check by running only its own new test file(s) (e.g. `npx jest src/__tests__/main/foo.test.ts`), and explicitly **not** to run the full suite, lint, build, or CI — those are integration steps you run once, after every sub-agent reports back.
...
5. **Verify each sub-agent's work before trusting it** — skim the diff for scope creep, skipped TDD, or lint-shaped problems. Fix small issues yourself rather than re-dispatching a sub-agent for them.
